                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zEAZY ought to be enough for anybody." - Bill Gates, 2017
    ui->zEAZYpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzEAZYSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzEAZY->setEnabled(false);
        ui->pushButtonMintzEAZY->setToolTip(tr("zEAZY is currently disabled due to maintenance."));

        ui->pushButtonSpendzEAZY->setEnabled(false);
        ui->pushButtonSpendzEAZY->setToolTip(tr("zEAZY is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zEAZYpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzEAZY_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zEAZY is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zEAZY...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zEAZY in ") +
...
void PrivacyDialog::on_pushButtonSpendzEAZY_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zEAZY is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zEAZY
        sendzEAZY();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zEAZY
    sendzEAZY();
}

void PrivacyDialog::on_pushButtonZEazyControl_clicked()
{
    ZEazyControlDialog* zEazyControl = new ZEazyControlDialog(this);
    zEazyControl->setModel(walletModel);
    zEazyControl->exec();
}

void PrivacyDialog::setZEazyControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzEazySelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzEAZY()
{
    ...
    double dAmount = ui->zEAZYpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zEAZY is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " EAZY </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zEAZY</b>";
    ...
    // use mints from zEazy selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZEazyControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZEazyControlDialog::GetSelectedMints();
    }
    ...
    // Clear zeazy selector in case it was used
    ZEazyControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zEazy Spend #: ") + QString::number(nCount) + ", ";
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zEAZY </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zEAZY "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zEAZY "));
    ui->labelzEAZYAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzEAZYSyncStatus->setVisible(fShow);
}
