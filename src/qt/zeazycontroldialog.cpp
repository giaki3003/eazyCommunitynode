...
#include "ui_zeazycontroldialog.h"
...
std::list<std::string> ZEazyControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZEazyControlDialog::listMints;

ZEazyControlDialog::ZEazyControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZEazyControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZEazyControlDialog::~ZEazyControlDialog()
{
    delete ui;
}

void ZEazyControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZEazyControlDialog::updateList()
{
...
void ZEazyControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZEazyControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZEazy_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZEazyControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZEazyControlDialog::GetSelectedMints()
{
...
void ZEazyControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
